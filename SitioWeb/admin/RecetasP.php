...
$contra="";
    ...
    $datos=$cliente->ListarPendientes();
    ...
    if (isset($_POST['aceptar']) && isset($_POST['id_receta'])) {
        $id_receta = $_POST['id_receta'];
        $cliente = new SoapClient(null, array('uri'=>'http://localhost/', 'location'=>'https://upmhproyect2024.000webhostapp.com/ServiciosWeb/servicioweb.php'));
        $respuesta = $cliente->AceptarRecetas($id_receta); 
        if ($respuesta != 'OK') {
            echo 'Receta Subida correctamente.';
        } else {
            echo 'Error al subir la receta.';
        }
    } 
...
<body>
  <header>
    ...
    <br>
    <h2 class="encabezado">Recetas recibidas</h2>
     <table border="1">
        <thead>
  <tr>
            <td><b>Clave</b></td>
        ...
        <?php 
            for ($rr = 0; $rr < count($datos); $rr++) { 
                ...
                echo '<td><form method="post" action="RecetasP.php" onsubmit="return confirm(\'¿Estás seguro de que deseas subir esta receta?\')">';
                echo '<input type="hidden" name="id_receta" value="'.$datos[$rr]["ID"].'">';
                echo '<button type="submit" name="aceptar">Aceptar</button></form>';
                
                echo '<form method="post" action="RecetasP.php" onsubmit="return confirm(\'¿Estás seguro de que deseas borrar esta receta?\')">';
                echo '<input type="hidden" name="id_receta" value="'.$datos[$rr]["ID"].'">';
                echo '<button type="submit" name="borrar">Borrar</button></form></td>';
