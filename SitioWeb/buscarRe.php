...
$contra="";
    ...
    if(isset($_POST["btnBuscar"]))
    {
        ...
        if(!empty($_REQUEST['txtBuscar']))
        {

            $termino=htmlspecialchars($_REQUEST['txtBuscar']);
            ...
            $datos=$cliente->BuscarReceta($termino);
...
<body>
    ...
    <section class="form-container_R">
    
        <form action="" method="post">
           <div>
           <input type="text" required maxlength="50" name="txtBuscar" placeholder="¿Que deseas buscar?" class="box" oninput="this.value = this.value.replace(/\s/g, '')">
           </div>
           <input type="submit" value="Buscar" class="btn" name="btnBuscar">
           <input type="reset" value="Borrar" class="btn" name="reset">
        </form>
...
<table class="content-table">
 ...
 <?php 
        if (!empty($datos) && (int)$datos[0]["ID"] != 0) {
